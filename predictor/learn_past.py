...
def trend_decompose(waittimedf):
...
def resid_predict_modeling(traindf):
...
def error_predict_modeling(datasetdf, N_lookbucks=6):
    ...
    for name, _ in df.groupby(level='year'):
        ...
        # make model
        model = resid_predict_modeling(traindf)
